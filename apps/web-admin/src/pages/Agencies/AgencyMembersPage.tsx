...
export default function AgencyMembersPage() {
  ...
  const [form, setForm] = useState({ userId: "", role: "agency_agent", status: "active" });
  ...
  useEffect(() => {
    ...
    const determinePermissions = async () => {
      ...
      try {
        ...
        const principal = data.principals?.find((p) => p.type === "agency" && p.id === agencyId);
        const allowed = principal?.role === "agency_admin" || principal?.role === "agency_manager";
  ...
  }, [tenantId, agencyId, canManageByRole]);
  ...
  const loadMembers = async () => {
    ...
    setError(null);
    try {
      const data = await listAgencyMembers(tenantId, agencyId);
  ...
  const submitMember = async () => {
    ...
    try {
      await addAgencyMember(tenantId, agencyId, {
        userId: form.userId.trim(),
        role: form.role,
        status: form.status
      });
      setForm({ userId: "", role: "agency_agent", status: "active" });
      await loadMembers();
    } catch (err: any) {
  ...
  const saveRow = async (membershipId: string) => {
    ...
    if (!patch) return;
    try {
      await patchAgencyMember(tenantId, agencyId, membershipId, patch);
      setRowEdits((prev) => {
        const next = { ...prev };
  ...
  return (
    <div className="space-y-3">
      <Link to={`/agencies/${agencyId}`} className="text-sm text-secondary hover:text-primary">
        ← Back to Agency
      </Link>
      <h1 className="text-2xl font-semibold text-primary">Agency Members</h1>
      <p className="text-sm text-secondary">Manage agency access roles.</p>
      {!tenantId ? (
