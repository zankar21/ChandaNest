...
import { getPublicProperty } from "../properties/properties.service";
import { curatedAnchors, type CuratedPlace } from "./curatedAnchors";
...
type NearbyCategoryConfig = {
  ...
  limit: number;
  types: string[];
  drivingFirst?: boolean;
  walkingFirst?: boolean;
};
...
const CATEGORY_CONFIG: Record<NearbyCategoryKey, NearbyCategoryConfig> = {
  atm: {
    title: "ATM",
    radiusMeters: 2000,
    limit: 1,
...
const CATEGORY_ORDER: NearbyCategoryKey[] = [
  "hospital",
  "school",
  "railway",
  "market",
...
function buildMapsUrl(lat: number, lng: number, placeId?: string) {
  const base = `https://www.google.com/maps/dir/?api=1&destination=${lat},${lng}`;
  return placeId ? `${base}&destination_place_id=${placeId}` : base;
}

function cacheDocId(propertyId: string, category: NearbyCategoryKey, placeId: string, mode: string) {
...
async function fetchPlaces(
  ...
  if (!res.ok || !payload) {
    logger.warn("Places API error", { status: res.status, type });
    return [];
  }
  if (payload.status && payload.status !== "OK" && payload.status !== "ZERO_RESULTS") {
    logger.warn("Places API response status", { status: payload.status, type });
    return [];
  }
  return Array.isArray(payload.results) ? payload.results : [];
}

function passesFilters(category: NearbyCategoryKey, place: any): boolean {
  ...
  if (category === "school") {
    if (reviews !== null && reviews < 25) return false;
    if (rating !== null && rating < 3.7) return false;
...
function fromPlacesResult(place: any, originLat: number, originLng: number): NearbyPlace | null {
...
async function resolveCategoryPlaces(
  category: NearbyCategoryKey,
  originLat: number,
  originLng: number,
  cityKey: string
): Promise<NearbyPlace[]> {
  const curated = curatedAnchors?.[cityKey]?.[category];
  ...
  const batches = await Promise.all(
    config.types.map((type) => fetchPlaces(originLat, originLng, config.radiusMeters, type))
  );
  const merged = new Map<string, NearbyPlace>();
  batches.flat().forEach((place) => {
    if (!passesFilters(category, place)) return;
...
function preferredMinutes(place: NearbyPlace, config: NearbyCategoryConfig) {
  if (config.walkingFirst && place.walk?.minutes !== null && place.walk?.minutes !== undefined) {
    if (place.walk.minutes <= 25) return place.walk.minutes;
  }
  if (config.drivingFirst && place.drive?.minutes !== null && place.drive?.minutes !== undefined) {
    return place.drive.minutes;
  }
  if (place.drive?.minutes !== null && place.drive?.minutes !== undefined) return place.drive.minutes;
  if (place.walk?.minutes !== null && place.walk?.minutes !== undefined) return place.walk.minutes;
  return 9999;
}

function scorePlace(place: NearbyPlace, config: NearbyCategoryConfig) {
  const preferred = preferredMinutes(place, config);
  const driveKm = place.drive?.km ?? place.distanceKm;
  return preferred + 0.2 * driveKm;
}

function tieBreak(a: NearbyPlace, b: NearbyPlace) {
...
async function resolveDistanceMatrix(
  propertyId: string,
  category: NearbyCategoryKey,
  ...
  const refs = places.map((place) =>
    firestore.collection(CACHE_COLLECTION).doc(cacheDocId(propertyId, category, place.placeId, mode))
  );
  ...
  let fetched: Map<string, CacheEntry> = new Map();
  if (missing.length > 0) {
    if (!env.googleMapsServerKey) {
      missing.forEach((place) => {
        ...
        fetched.set(place.placeId, {
          ...
          minutes: null,
          status: "FALLBACK",
          fetchedAt: admin.firestore.Timestamp.fromDate(now),
          expiresAt: admin.firestore.Timestamp.fromDate(new Date(now.getTime() + CACHE_TTL_MS))
        });
      });
    } else {
      ...
      url.searchParams.set(
        "destinations",
        missing.map((place) => `${place.location.lat},${place.location.lng}`).join("|")
      );
      ...
      try {
        ...
        elements.forEach((element, idx) => {
          ...
          if (element?.status === "OK" && element.distance?.value !== undefined) {
            ...
            const km = roundKm(
              haversineKm(origin.lat, origin.lng, place.location.lat, place.location.lng)
            );
            fetched.set(place.placeId, {
              propertyId,
              category,
        ...
        logger.warn("Distance Matrix failed, using fallback", err);
        missing.forEach((place) => {
          const km = roundKm(haversineKm(origin.lat, origin.lng, place.location.lat, place.location.lng));
          fetched.set(place.placeId, {
            propertyId,
  ...
  fetched.forEach((value, key) => {
    const ref = firestore.collection(CACHE_COLLECTION).doc(cacheDocId(propertyId, category, key, mode));
  ...
  const all = new Map<string, CacheEntry>();
  cached.forEach((value, key) => all.set(key, value));
  fetched.forEach((value, key) => all.set(key, value));
  return all;
}

export async function getPublicNearby(propertyId: string) {
  const property = (await getPublicProperty(propertyId)) as any;
  ...
  const categoryBuckets: Record<NearbyCategoryKey, NearbyPlace[]> = {
    atm: [],
    grocery: [],
    pharmacy: [],
    school: [],
  ...
  const selected: { category: NearbyCategoryKey; place: NearbyPlace }[] = [];
  for (const category of CATEGORY_ORDER) {
    if (selected.length >= MAX_DISTANCE_MATRIX_DESTINATIONS) break;
  ...
  const grouped = new Map<NearbyCategoryKey, NearbyPlace[]>();
  ...
  for (const category of Array.from(grouped.keys())) {
    ...
    places.forEach((place) => {
      ...
      if (drive) place.drive = { km: drive.km, minutes: drive.minutes, status: drive.status };
      if (walk) place.walk = { km: walk.km, minutes: walk.minutes, status: walk.status };
    });
    const config = CATEGORY_CONFIG[category];
    const ranked = places
      .slice()
